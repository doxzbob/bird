...
f_run(struct filter_slot *filter_slot, struct rte **rte, struct linpool *tmp_pool, int flags)
{
  ...
  enum filter_return fret = interpret(&filter_state, filter_slot->filter->root, NULL);
  ...
  if (fret < F_ACCEPT) {
    if (!(filter_state.flags & FF_SILENT))
      log_rl(&rl_runtime_err, L_ERR "Filter %s did not return accept nor reject. Make up your mind", filter_name(filter_slot->filter));
    return F_ERROR;
  }
  DBG( "done (%u)\n", res.val.i );
  return fret;
  ...
  WALK_LIST(sym, config->symbols) {
      ...
      case SYM_PROTO:
	{
	  ...
	  WALK_LIST(c, sym->proto->proto->channels) {
	    ...
	    if (c->in_filter.filter == FILTER_ACCEPT)
	      debug(" ALL\n");
	    else if (c->in_filter.filter == FILTER_REJECT)
	      debug(" NONE\n");
	    else if (c->in_filter.filter == FILTER_UNDEF)
	      debug(" UNDEF\n");
	    else if (c->in_filter.filter->sym) {
	      ASSERT(c->in_filter.filter->sym->filter == c->in_filter.filter);
	      debug(" named filter %s\n", c->in_filter.filter->sym->name);
	    } else {
	      debug("\n");
	      f_dump_line(c->in_filter.filter->root, 2);
	    }
	  }
	}
    }
